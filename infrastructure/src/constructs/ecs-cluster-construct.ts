...
export class EcsClusterConstruct extends Construct {
  ...
  constructor(scope: Construct, id: string, props: EcsClusterConstructProps) {
    ...
    const scalingTopic = new sns.Topic(this, 'clusterScalingTopic', {
    ...
    if (!props.isFargateCluster) {
      ...
      const autoScalingGroup = new asg.AutoScalingGroup(this, `${id}-cluster-asg`, {
        vpc: props.vpc,
      ...
      const capacityProvider = new ecs.AsgCapacityProvider(this, `${id}-asgcapacity`, {
        autoScalingGroup,
      });      
    ...
    new ssm.StringParameter(this, `${id}-param`, {
      allowedPattern: ".*",
      description: "Param for ECS Cluster of " + id,
      parameterName: `/${props.config.AppPrefix}/${id}`,
