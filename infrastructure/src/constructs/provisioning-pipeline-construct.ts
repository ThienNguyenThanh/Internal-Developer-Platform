...
export class ProvisioningPipelineConstruct extends Construct {
  ...
  constructor(parent: Construct, name: string, props: ProvisioningPipelineConstructProps) {
    ...
    const serializeEnvironmentVariablesFunction = new lambdaNodeJs.NodejsFunction(this, "serialize-env-variables", {
      entry: "./lambda/serializeEnvironmentVariables/serializeEnvironmentVariables.ts",
      runtime: lambda.Runtime.NODEJS_18_X,
    });

    const serializeEnvVariablesStep = new stepFunctionsTasks.LambdaInvoke(this, "Serialize Env Vars", {
    ...
    const packageBoilerplateProject = new codeBuild.Project(this, "buildProject", {
      environment: {
        ...
        environmentVariables: {
          HOST_NAME: {
            ...
            value: props.config.R53HostedZoneName,
          },
        },
      },
      vpc: props.vpc,
    ...
    const packageBoilerplateStep = new stepFunctionsTasks.CodeBuildStartBuild(this, "Package Boilerplate", {
    ...
    const deployCustomCFProject = new codeBuild.Project(this, "deploy-custom-cf-project", {
      environment: {
        buildImage: codeBuild.LinuxBuildImage.STANDARD_6_0,
        privileged: true,
        computeType: codeBuild.ComputeType.MEDIUM,
    ...
    const deployBoilerplateStep = new CodeBuildStartBuildExtended(this, "Deploy Boilerplate", {
      resultPath: "$.Result",
      integrationPattern: stepFunctions.IntegrationPattern.RUN_JOB,
    ...
    const aggregateOutputFunction = new lambdaNodeJs.NodejsFunction(this, "aggregate-output", {
    ...
    const aggregateOutputStep = new stepFunctionsTasks.LambdaInvoke(this, "Aggregate Output", {
    ...
    const packageBoilerplateMap = new stepFunctions.Map(this, "Package Boilerplate Map", {
    ...
    const deployBoilerPlateMap = new stepFunctions.Map(this, "Deploy Boilerplate Map", {
      itemsPath: stepFunctions.JsonPath.stringAt("$.repositories"),
      resultPath: "$.Result",
      parameters: {
        "repository.$": "$$.Map.Item.Value",
    ...
    const provisioningPipelineStateMachine = new stepFunctions.StateMachine(this, "provisioningPipelineStateMachine", {
